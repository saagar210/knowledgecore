...
| Trust identity/session | 1 | `spec/39-managed-identity-oidc-device-cert-v1.md` | kc_core | kc_cli/src-tauri/ui | 1-adj | deterministic OIDC claim subset canonical JSON, session selection ordering (`created_at_ms`, `session_id`), and deterministic certificate chain hash derivation (`cert_id`, `device_id`, `fingerprint`) | additive optional identity metadata fields ok | bump on claim subset normalization, session ordering, or chain-hash derivation semantics |
