...
| Sync snapshots/head/conflict | 2 | `spec/31-sync-s3-transport-v1.md` + `spec/33-cross-device-passphrase-trust-v1.md` + `spec/35-device-trust-manual-verify-v1.md` | kc_core | kc_cli/src-tauri/ui | 1 | deterministic snapshot id derivation `kc.sync.snapshot.v2`; deterministic head/conflict serialization; trust metadata required for schema_version=2 heads; optional author identity fields (`author_device_id`, `author_fingerprint`, `author_signature`) are deterministic when present; no auto-merge | additive optional fields ok when schema_version preserved | bump on snapshot id, trust model, lock protocol, or conflict semantics change |
...
| RPC envelope | 1 | `spec/19-tauri-rpc-surface.md` | src-tauri | UI | 1-adj | strict one-of envelope; methods include lock-session RPC (`vault_lock_status`, `vault_unlock`, `vault_lock`), `ingest_inbox_start/stop`, `vault_encryption_status/enable/migrate`, `vault_recovery_status/generate/verify`, lineage v2 + overlay RPCs (`lineage_query_v2`, `lineage_overlay_add/remove/list`); deterministic reqs carry caller-controlled timestamps (`now_ms`/`created_at_ms`) | additive methods ok | bump on breaking change |
