...
| Sync merge preview report | 2 | `spec/37-sync-conservative-auto-merge-v1.md` | kc_core | kc_cli/src-tauri/ui | 1 | deterministic normalization/sorting for change-set arrays; deterministic overlap/reasons ordering; v2 adds deterministic `decision_trace` and conflict categories (`trust_chain_mismatch`, `lineage_lock_conflict`) for `conservative_plus_v2`; policies only allow disjoint object hashes + disjoint lineage overlay ids with trust/lock preconditions | read v1 + v2 during transition; emit v2 when policy=`conservative_plus_v2` | bump on merge policy semantics, ordering rules, or overlap logic |
...
| Lineage governance (roles + locks) | 2 | `spec/41-lineage-governance-rbac-v2.md` + `spec/38-lineage-collab-turn-lock-v1.md` | kc_core | kc_cli/src-tauri/ui | 1-adj | deterministic RBAC precedence by `role_rank,subject_id,role_name`; overlay mutation requires both lock + `lineage.overlay.write`; scoped lock schemas support `scope_kind` in `{doc,set}` with deterministic token/status serialization | read v1 per-doc lock status during transition; emit v2 governance artifacts for new role/scoped-lock flows | bump on role precedence, permission semantics, scope enum, or lock/token derivation |
