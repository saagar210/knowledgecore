...
| RPC envelope | 1 | `spec/19-tauri-rpc-surface.md` | src-tauri | UI | 1-adj | strict one-of envelope; methods include lock-session RPC (`vault_lock_status`, `vault_unlock`, `vault_lock`), `ingest_inbox_start/stop`, `vault_encryption_status/enable/migrate`, lineage v2 + overlay RPCs (`lineage_query_v2`, `lineage_overlay_add/remove/list`); deterministic reqs carry caller-controlled timestamps (`now_ms`/`created_at_ms`) | additive methods ok | bump on breaking change |
