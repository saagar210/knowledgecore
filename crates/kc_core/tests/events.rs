...
fn events_chain_hashes_deterministically() {
    ...
    let first = append_event(&conn, 10, "test.first", &serde_json::json!({"k": 1})).expect("first event");
    let second = append_event(&conn, 11, "test.second", &serde_json::json!({"k": 2})).expect("second event");
