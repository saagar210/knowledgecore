...
#[derive(Debug, Clone)]
pub struct AzureRecoveryEscrowConfig {
...
#[derive(Debug, Clone)]
pub struct AzureRecoveryEscrowProvider {
    pub config: AzureRecoveryEscrowConfig,
}

impl AzureRecoveryEscrowProvider {
    pub fn new(config: AzureRecoveryEscrowConfig) -> Self {
        Self { config }
    }

    fn emulation_root(&self) -> Option<PathBuf> {
        std::env::var("KC_RECOVERY_ESCROW_AZURE_EMULATE_DIR")
            .ok()
...
impl RecoveryEscrowProvider for AzureRecoveryEscrowProvider {
    fn provider_id(&self) -> &str {
        "azure"
    }

    fn status(&self) -> AppResult<RecoveryEscrowProviderStatus> {
        let configured = !self.config.key_vault_url.trim().is_empty()
        ...
        .map_err(|e| {
            AppError::new(
                ...
                "recovery",
                "failed serializing azure escrow provider status",
                false,
    ...
    fn write(&self, req: RecoveryEscrowWriteRequest<'_>) -> AppResult<RecoveryEscrowDescriptorV2> {
        let Some(root) = self.emulation_root() else {
            return Err(AppError::new(
                "KC_RECOVERY_ESCROW_UNAVAILABLE",
                "recovery",
                "azure recovery escrow provider is unavailable in this runtime",
                false,
                serde_json::json!({
                    "provider": "azure",
                    "hint": "configure KC_RECOVERY_ESCROW_AZURE_EMULATE_DIR for deterministic local adapter tests"
                }),
            ));
        };
        ...
        if let Some(parent) = path.parent() {
            fs::create_dir_all(parent).map_err(|e| {
                AppError::new(
                    ...
                    "recovery",
                    "failed creating azure emulation escrow directory",
                    false,
        ...
        fs::write(&path, req.key_blob).map_err(|e| {
            AppError::new(
                "KC_RECOVERY_ESCROW_WRITE_FAILED",
                "recovery",
                "failed writing azure emulation escrow payload",
                false,
    ...
    fn read(&self, req: RecoveryEscrowReadRequest<'_>) -> AppResult<Vec<u8>> {
        ...
        if req.descriptor.provider != self.provider_id() {
            return Err(AppError::new(
                ...
                "recovery",
                "escrow descriptor provider does not match azure adapter",
                false,
                serde_json::json!({
        ...
        let bytes = fs::read(&path).map_err(|e| {
            AppError::new(
                "KC_RECOVERY_ESCROW_RESTORE_FAILED",
                "recovery",
                "failed reading azure emulation escrow payload",
                false,
                serde_json::json!({ "error": e.to_string(), "path": path }),
            )
        })?;
