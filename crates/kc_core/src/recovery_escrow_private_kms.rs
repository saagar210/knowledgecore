...
#[derive(Debug, Clone)]
pub struct PrivateKmsRecoveryEscrowConfig {
    pub endpoint: String,
...
#[derive(Debug, Clone)]
pub struct PrivateKmsRecoveryEscrowProvider {
    pub config: PrivateKmsRecoveryEscrowConfig,
}

impl PrivateKmsRecoveryEscrowProvider {
    pub fn new(config: PrivateKmsRecoveryEscrowConfig) -> Self {
        Self { config }
    }

    fn emulation_root(&self) -> Option<PathBuf> {
        std::env::var("KC_RECOVERY_ESCROW_PRIVATE_KMS_EMULATE_DIR")
            .ok()
...
impl RecoveryEscrowProvider for PrivateKmsRecoveryEscrowProvider {
    fn provider_id(&self) -> &str {
        "private_kms"
    }

    fn status(&self) -> AppResult<RecoveryEscrowProviderStatus> {
        let configured = !self.config.endpoint.trim().is_empty()
            && !self.config.key_alias.trim().is_empty()
            && !self.config.tenant.trim().is_empty()
        ...
        .map_err(|e| {
            AppError::new(
                ...
                "recovery",
                "failed serializing private_kms escrow provider status",
                false,
    ...
    fn write(&self, req: RecoveryEscrowWriteRequest<'_>) -> AppResult<RecoveryEscrowDescriptorV2> {
        let Some(root) = self.emulation_root() else {
            return Err(AppError::new(
                "KC_RECOVERY_ESCROW_UNAVAILABLE",
                "recovery",
                "private_kms recovery escrow provider is unavailable in this runtime",
                false,
                serde_json::json!({
                    "provider": "private_kms",
                    "hint": "configure KC_RECOVERY_ESCROW_PRIVATE_KMS_EMULATE_DIR for deterministic local adapter tests"
                }),
            ));
        };
        ...
        if let Some(parent) = path.parent() {
            fs::create_dir_all(parent).map_err(|e| {
                AppError::new(
                    ...
                    "recovery",
                    "failed creating private_kms emulation escrow directory",
                    false,
        ...
        fs::write(&path, req.key_blob).map_err(|e| {
            AppError::new(
                "KC_RECOVERY_ESCROW_WRITE_FAILED",
                "recovery",
                "failed writing private_kms emulation escrow payload",
                false,
        ...
        let descriptor = RecoveryEscrowDescriptorV2 {
            ...
            key_id: format!(
                "privatekms:{}/{}/{}",
                self.config.tenant, self.config.endpoint, self.config.key_alias
            ),
    ...
    fn read(&self, req: RecoveryEscrowReadRequest<'_>) -> AppResult<Vec<u8>> {
        ...
        if req.descriptor.provider != self.provider_id() {
            return Err(AppError::new(
                ...
                "recovery",
                "escrow descriptor provider does not match private_kms adapter",
                false,
                serde_json::json!({
        ...
        let bytes = fs::read(&path).map_err(|e| {
            AppError::new(
                "KC_RECOVERY_ESCROW_RESTORE_FAILED",
                "recovery",
                "failed reading private_kms emulation escrow payload",
                false,
                serde_json::json!({ "error": e.to_string(), "path": path }),
            )
        })?;
